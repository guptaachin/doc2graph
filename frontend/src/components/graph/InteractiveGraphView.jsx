  ...
  const fetchGraphData = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes('Failed to fetch') || err.message.includes('fetch')) {
        userMessage = "Neo4j database is unreachable. Please wait at least 30 seconds for Neo4j to start, then click Retry.";
      } else if (err.message.includes('500') || err.message.includes('503')) {
        userMessage = "Database connection failed. Neo4j may still be starting up. Please wait 30 seconds and try again.";
      } else if (err.message.includes('404')) {
        userMessage = "Graph service is not available. Please ensure the backend is running properly.";
      }
